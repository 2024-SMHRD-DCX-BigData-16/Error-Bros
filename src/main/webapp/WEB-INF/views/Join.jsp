				...
				.ajax({
					...
					success : function(result) {
						if (result) {
							...
							document.getElementById("idMessage").style.color = "green"; // 사용 가능 시 초록색 표시
						} else {
							document.getElementById("idMessage").innerHTML = "이미 사용 중인 아이디입니다.";
							document.getElementById("idMessage").style.color = "red"; // 사용 불가능 시 빨간색 표시
						}
					}
				});
	}
				...
				.ajax({
					...
					success : function(result) {
						if (result) {
							...
							document.getElementById("emailMessage").style.color = "green"; // 사용 가능 시 초록색 표시
						} else {
							document.getElementById("emailMessage").innerHTML = "이미 사용 중인 이메일입니다.";
							document.getElementById("emailMessage").style.color = "red"; // 사용 불가능 시 빨간색 표시
						}
					}
				});
	}
...
<body>
	...
	<div class="center-container">
		<form class="signup-form" name="insertMember" action="insertMember"
			method="post">
			<h2>회원가입</h2>
			<input type="text" id="mem_id" name="mem_id" placeholder="아이디"
				required>
			<button type="button" onclick="checkDuplicateId()">아이디 중복 확인</button>
			<span id="idMessage"></span> <input type="password" name="mem_pw"
				placeholder="비밀번호" required> <input type="text"
				name="mem_nm" placeholder="이름" required> <input type="text"
				name="mem_phone" placeholder="전화번호" required> <input
				type="email" id="mem_email" name="mem_email" placeholder="이메일"
				required>
			<button type="button" onclick="checkDuplicateEmail()">이메일 중복
				확인</button>
			<span id="emailMessage"></span> <input type="date"
				name="mem_birthdate" required> <input type="text"
				name="mem_role" placeholder="회원 역할" required> <select
				name="mem_gender">
